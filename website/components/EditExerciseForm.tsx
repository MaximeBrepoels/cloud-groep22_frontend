...
const EditExerciseForm: React.FC = () => {
    ...
    const deleteExercise = () => {
        if (workoutIdNumber === undefined || isNaN(workoutIdNumber) || exerciseIdNumber === undefined || isNaN(exerciseIdNumber)) return;
        exerciseService
            .deleteExerciseFromWorkout(workoutIdNumber, exerciseIdNumber)
            .then(() => router.push({ pathname: "/edit-workout", query: { workoutId } }))
    ...
    const updateExercise = () => {
        if (exerciseIdNumber === undefined || isNaN(exerciseIdNumber)) return;
        exerciseService
    ...
    return (
        ...
        >
            ...
            <div className="flex gap-2 mb-2 w-full">
                <button
                    type="button"
                    className={`flex-1 border rounded p-2 ${autoIncrease ? "bg-gray-200" : ""}`}
                    ...
                    <div className="flex gap-2 mb-2 w-full">
                        {[1.05, 1.1, 1.15].map((factor) => (
                            <button
                                key={factor}
                                type="button"
                                className={`flex-1 border rounded p-2 ${increaseFactor === factor ? "bg-gray-200" : ""}`}
